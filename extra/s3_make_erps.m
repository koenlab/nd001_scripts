...
for r = 1:length(reference_list)
    for pari = 1:length(participant_list)
        ...
        % Load EEG set file
        if r == 1       %for average re-reference
            eeg_file = 'set10_goodepochs_pre250blc_avgref_0.1hzhpf_500Hz.set';
        elseif r == 2   %for mastoid re-reference
            eeg_file = 'set11_goodepochs_pre250blc_mastoidref_0.1hzhpf_500Hz.set';
        end
        ...
        % Save filtered ERP
        ERP.erpname  = strcat('erp_30hzlpf_-250to1500_', reference_list{r}, 'ref_preblc');
        ...
        % Save ERP
        ERP.erpname  = strcat('erp_30hzlpf_-250to1500_', reference_list{r}, 'ref_preblc_binop_trialtype');
        ERP.filename = sprintf('%s.erp',ERP.erpname);
        ERP.filepath = par_erps_out_directory;
        ERP.subject  = participant;
        pop_savemyerp( ERP );
        ...
        % Save ERP
        ERP.erpname  = strcat('erp_30hzlpf_-250to1500_', reference_list{r}, 'ref_preblc_binop_SME');
        ERP.filename = sprintf('%s.erp',ERP.erpname);
        ERP.filepath = par_erps_out_directory;
        ERP.subject  = participant;
        pop_savemyerp( ERP );
        ...
        % Save ERP
        ERP.erpname  = strcat('erp_30hzlpf_-250to1500_', reference_list{r}, 'ref_preblc_binop_condmem');
        ERP.filename = sprintf('%s.erp',ERP.erpname);
        ERP.filepath = par_erps_out_directory;
        ERP.subject  = participant;
        pop_savemyerp( ERP );
        
        %% Topo Plots for Conditional Memory
        
        % Bin by conditional memory aka AB|AChit vs. AB|ACmiss
        cfg = [];
        cfg.bins = 1:2; % This is the scene and object bins, respectively
        cfg.channels = 1:ERP.nchan;
        cfg.blc        = 'pre'; % Although this should already be done, make sure it is
        ERP = pop_ploterps( ERP,  cfg.bins,  cfg.channels , 'Blc', cfg.blc, ....
            'AutoYlim', 'on', 'Axsize', [ 0.05 0.08], 'BinNum', 'on', 'ChLabel', 'on',...
        ...
        f.Position = [500 50 1100 1000];
        f.PaperPositionMode = 'auto';
        saveas(f,fullfile(par_fig_out_directory,strcat('erp_', reference_list{r}, 'ref_AB_SME-AC_ERP_topo.png')));
        if ~examine_figures, close(f); end
