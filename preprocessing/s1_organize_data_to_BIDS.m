...
directories.functions   = fullfile(directories.top, 'scripts', 'functions');
...
% BIDS file names
bids_files.participants_tsv = fullfile(directories.top,'raw_bids','participants.tsv');
...
for pari = 1:length(participant_list)
    ...
    par_data_directory         = fullfile(directories.raw, participant);
...
%     
%     %Load EEG Data
%     %Load and merge the two separate EEG files for Subject 130
%     if strcmp(participant, 'sub-130')
%         eeg1_vhdr   = sprintf('%s_task-study_eeg.vhdr', participant);
%         eeg1_set    = sprintf('%s_task-study_eeg.set', participant);
%         INEEG1      = pop_loadbv(par_source_eeg_directory, eeg1_vhdr, [], []);
%         
%         eeg2_vhdr   = sprintf('%s_task-study2_eeg.vhdr', participant);
%         eeg2_set    = sprintf('%s_task-study2_eeg.set', participant);
%         INEEG2      = pop_loadbv(par_source_eeg_directory, eeg2_vhdr, [], []);
%         
%         EEG = pop_mergeset(INEEG1, INEEG2);
%         eeg_set     = sprintf('%s_task-study_eeg.vhdr', participant);
%     else
%         
%     %Load EEG Data for all other subjects with one file
