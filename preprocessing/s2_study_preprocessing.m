...
user_def_badchans.sub_105 = {'CP2' 'C2' 'FC2' 'P4'};
user_def_badchans.sub_106 = {'FC3'};
user_def_badchans.sub_108 = {'C5'};
user_def_badchans.sub_109 = {'Cz'};
user_def_badchans.sub_122 = {'Cz'};
user_def_badchans.sub_125 = {'F2'}; % Looks bad on ERPs
user_def_badchans.sub_128 = {'CP4' 'CP1'};
user_def_badchans.sub_131 = {'AF7' 'C5' 'P7'};

%% Options for loading prior information
% True/False flags for loading previous bad channels and marks
load_bad_chans     = true; % Load bad channels
load_preica_marks  = true; % Load manual marked epochs preica
reinspect_preica   = false; % If true, and load_preica is TRUE, reimspect marked epochs
load_postica_marks = false; % Load all marked epochs from postica
...
for pari = 1:length(participant_list)
    ...
    make_dirs({par_preproc_out_dir par_fig_out_dir par_sets_out_dir ...
        par_erps_out_dir par_elist_out_dir par_sets_out_dir});
    
    % Load EEG set file
    eeg_file = sprintf('%s_task-study_eeg.set', participant);
    EEG      = pop_loadset( eeg_file, par_data_eeg_dir );
    ...
    cfg.eventlist       = fullfile( par_elist_out_dir, 'cont_eeg_type_membin_elist.txt' );
    ...
    cfg.exportEL  = fullfile( par_elist_out_dir, 'cont_eeg_type_membin_elist.txt' ); % Overwrite above file
    ...
    save( fullfile( par_preproc_out_dir, 'orig_chanlocs.mat' ), 'chanlocs' );
    ...
    %% Step 4: Band-Pass Filter continuous EEG data
    % BANDPASS FILTER - We are performing a band pass filter on the
    ...
    % 0.05 Hz to 100Hz bandpass for ERP computation       
    cfg = [];
    ...
    % Save EEG Data
    cfg = [];
    cfg.setname  = 'set04_0.05-100bpf_500Hz';
    cfg.path     = par_sets_out_dir;
    cfg.filename = [cfg.setname '.set'];
    EEG = save_eeglab_set( EEG, cfg.setname, cfg.path, cfg.filename );  
    
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set04_0.1hzhpf_500Hz.set');
    ...
    % Save EEG
    cfg = [];
    cfg.setname  = 'set05_binepochs_wholeblc_0.05-100bpf_500Hz';
    cfg.path     = par_sets_out_dir;
    cfg.filename = [cfg.setname '.set'];
    EEG = save_eeglab_set( EEG, cfg.setname, cfg.path, cfg.filename );
    
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set05_binepochs_wholeblc_0.1hzhpf_500Hz.set');
    if exist(old_file,'file')
        delete(old_file);
    end

    %% Step 6: Detect Artifacted Epochs and Bad Channels (using 0.05-100HZ BPF data)
    ...
    if load_bad_chans
        ...
        load(fullfile(par_preproc_out_dir,'bad_channels.mat'));
        par_field = strrep(participant,'-','_');
        if isfield(user_def_badchans, par_field )
            other_bad = find( ismember( {EEG.chanlocs.labels}, user_def_badchans.(par_field) ) );
            cfg.badchans = unique([bad_channels.bad_inds other_bad]);
        end
    ...
    % Load preica marks
    if load_preica_marks
        
        % Load preica_marked_epochs.mat
        load( fullfile( par_preproc_out_dir, 'preica_marked_epochs.mat') );
    ...
    % Run visual inspection if needed
    orig_EEG = EEG;
    if (reinspect_preica && load_preica_marks) || ~load_preica_marks
    ...
    save( fullfile( par_preproc_out_dir, 'bad_channels.mat' ), 'bad_channels', 'bad_chanids', 'bad_chanlabels' );
    
    % Save the preica_marked_epochs
    save( fullfile( par_preproc_out_dir, 'preica_marked_epochs.mat') , 'preica_marked_epochs' );
    ...
    cfg = [];
    cfg.elname = fullfile( par_elist_out_dir, 'cont_eeg_type_membin_elist.txt' ); % Overwrite existing event_list file
    EEG = pop_exporteegeventlist( EEG, cfg.elname );
    
    % Save EEG
    cfg = [];
    cfg.setname  = 'set06_artmark_binepochs_0.05-100bpf_500Hz';
    cfg.path     = par_sets_out_dir;
    cfg.filename = [cfg.setname '.set'];
    EEG = save_eeglab_set( EEG, cfg.setname, cfg.path, cfg.filename );    
    
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set06_artmark_binepochs_0.1hzhpf_500Hz.set');
    if exist(old_file,'file')
        delete(old_file);
    end
    ...
    % Load set03 to re-process with a 1-100hz bpf
    eeg_file = 'set03_binlist_resampled_500Hz.set';
    ...
    while true
        ...
        for loopi = 1:n_loops
            ...
            if loopi ~= n_loops
                ...
                saveas(gcf,fullfile(par_fig_out_dir,sprintf('ica_comps%d-%d.png',comp_start,comp_end)));
            else
                EEG = pop_selectcomps( EEG, comp_start:n_ics );
                saveas(gcf,fullfile(par_fig_out_dir,sprintf('ica_comps%d-%d.png',comp_start,n_ics)));
            end
    ...
    save( fullfile( par_preproc_out_dir, 'ica_data_output.mat' ), vars2save{:} );
    
    % Clear EEG
    EEG = [];
    ALLEEG = [];
    
    %% Step 10: Apply IC results to 0.05-100 Hz Data and interpolate bad channels (if missing)
    % Load EEG set file
    eeg_file = 'set06_artmark_binepochs_0.05-100bpf_500Hz.set';
    EEG      = pop_loadset( eeg_file, par_sets_out_dir );
    ...
    % Copy ICA information to loaded .01-100 Hz filtered data
    ...
    % Save EEG
    cfg = [];
    cfg.setname  = 'set07_icaclean_binepochs_0.05-100bpf_500Hz';
    cfg.path     = par_sets_out_dir;
    cfg.filename = [cfg.setname '.set'];
    EEG = save_eeglab_set( EEG, cfg.setname, cfg.path, cfg.filename );    
    
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set07_icaclean_binepochs_0.1hzhpf_500Hz.set');
    if exist(old_file,'file')
        delete(old_file);
    end
    ...
    % Save the set
    cfg = [];
    cfg.setname  = 'set08_avgref_icaclean_binepochs_0.05-100bpf_500Hz';
    cfg.path     = par_sets_out_dir;
    cfg.filename = [cfg.setname '.set'];
    EEG = save_eeglab_set( EEG, cfg.setname, cfg.path, cfg.filename );
    
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set08_averageref_icacleaned_binepochs_0.1hzhpf_500Hz.set');
    if exist(old_file,'file')
        delete(old_file);
    end
    ...
    cfg = [];
    cfg.baseline_time  = [-200 0]; % Epoch time window start and end
    EEG = pop_rmbase( EEG , cfg.baseline_time );
    
    % Save the set
    cfg = [];
    cfg.setname  = 'set09_pre200blc_avgref_icacleaned_binepochs_0.05-100bpf_500Hz';
    cfg.path     = par_sets_out_dir;
    cfg.filename = [cfg.setname '.set'];
    EEG = save_eeglab_set( EEG, cfg.setname, cfg.path, cfg.filename );
    
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set09_pre250blc_avgref_icacleaned_binepochs_0.1hzhpf_500Hz.set');
    if exist(old_file,'file')
        delete(old_file);
    end
    ...
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set10_goodepochs_pre250blc_avgref_0.1hzhpf_500Hz.set');
    if exist(old_file,'file')
        delete(old_file);
    end
    ...
    % Remove 0.1Hz data
    old_file = fullfile(par_sets_out_dir, 'set11_goodepochs_pre250blc_mastoidref_0.1hzhpf_500Hz.set');
    if exist(old_file,'file')
        delete(old_file);
    end
